...
Models are configured through the `{{ config() }}` macro at the top of the file:
...
| `pre-hook` | SQL to run before model | `"{{ create_ingestion_table(this) }}"` |
| `post-hook` | SQL to run after model | `"GRANT SELECT ON {{ this }} TO ROLE SYSADMIN"` |
